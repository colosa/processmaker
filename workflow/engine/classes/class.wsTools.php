...
class workspaceTools
{
    ...
    public function initPropel ($root = false)
    {
        ...
        $config = array ('datasources' => array ('workflow' => array ('connection' => $wfDetails["dsn"],'adapter' => $wfDetails["adapter"] ),'rbac' => array ('connection' => $rbDetails["dsn"],'adapter' => $rbDetails["adapter"]),'rp' => array ('connection' => $rpDetails["dsn"],'adapter' => $rpDetails["adapter"])));
        if ($root) {
    ...
    static public function restore ($filename, $srcWorkspace, $dstWorkspace = NULL, $overwrite = true)
    {
        ...
        //Search for metafiles in the new standard (the old standard would contain txt files).
